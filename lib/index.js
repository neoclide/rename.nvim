...
let RenamePlugin = class RenamePlugin {
    ...
    renameSearch(args) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let { nvim } = this;
            let exe = yield nvim.call('rename#get_execute');
            if (!exe)
                return;
            let cmd = exe.endsWith('ag') ? 'ag' : 'rg';
            let extraArgs = yield nvim.getVar('rename_search_extra_args');
            if (Array.isArray(extraArgs) && extraArgs.length) {
                args = extraArgs.concat(args);
            }
            let opts = option_1.convertOptions(args, cmd);
            let useVcs = yield nvim.getVar('rename_search_vcs_root');
            let cwd = yield nvim.call('getcwd');
            let buffer = yield nvim.buffer;
            let iskeyword = yield buffer.getOption('iskeyword');
            if (useVcs) {
                let full_path = yield nvim.call('rename#get_fullpath');
                let dir = full_path ? path.dirname(full_path) : cwd;
                dir = index_1.findVcsRoot(dir);
                if (dir)
                    cwd = dir;
            }
            let search = new search_1.default(nvim, cmd, opts, cwd, iskeyword);
            yield search.start();
    ...
    renameBufferUnload(args) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (search) {
                yield search.stop(true);
                let idx = this.searches.findIndex(o => o === search);
                this.searches.splice(idx, 1);
            }
        });
    }
    renameSearchWrite(args) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let bufnr = args[0];
            let { nvim } = this;
            let search = this.searches.find(o => o.bufnr == bufnr);
            yield nvim.command('doautocmd BufWrite,BufWritePre');
            if (search) {
                yield search.writeFiles();
            }
            yield nvim.command('doautocmd BufWritePost');
        });
    }
    ...
    openFile(cmd) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield nvim.command(`${cmd} +${lnum} ${file}`);
        });
    }
    onMove(moveType) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!search)
                return;
            let p = yield this.nvim.call('getcurpos');
            let lnum = p[1];
            let col = p[2];
            let arr;
            switch (moveType) {
            ...
            if (arr) {
                ...
                yield this.nvim.call('cursor', [lnum, col + 1]);
...
tslib_1.__decorate([
    neovim_1.Function('RenameCancel', { sync: true })
...
tslib_1.__decorate([
    neovim_1.Function('RenameSearchWrite', { sync: true })
], RenamePlugin.prototype, "renameSearchWrite", null);
tslib_1.__decorate([
